...
const main = async ({
  ...
  console.log(`02 - Deploying MATIC Call Theta Vault 03 on ${network.name}`);
  ...
  const pricer = await deploy("OptionsPremiumPricerMatic03", {
    from: deployer,
    contract: {
  ...
  console.log(`RibbonThetaVaultMATICCall03 pricer @ ${pricer.address}`);

  // Can't verify pricer because it's compiled with 0.7.3

  /**const strikeSelection = await deploy("StrikeSelectionMATIC02", {
  ...
  const initArgs = [
    {
      ...
      _strikeSelection: '0x1C490CcbE4D9312b82Bbf6e7F55dFe1D307a6781',
  ...
  const proxy = await deploy("RibbonThetaVaultMATICCall03", {
    contract: "AdminUpgradeabilityProxy",
    from: deployer,
    args: [logicDeployment.address, admin, initData],
  });

  console.log(`RibbonThetaVaultMATICCall03 Proxy @ ${proxy.address}`);
...
main.tags = ["RibbonThetaVaultMATICCall03"];
