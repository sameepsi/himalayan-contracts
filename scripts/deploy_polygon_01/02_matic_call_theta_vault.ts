...
const TOKEN_NAME = {
  [CHAINID.POLYGON_MAINNET]: "Himalayan MATIC Vault 10",
...
const TOKEN_SYMBOL = {
  [CHAINID.POLYGON_MAINNET]: "hMATIC10",
...
const main = async ({
  ...
  const strikeSelection = await deploy("StrikeSelectionMATIC10", {
  ...
  console.log(
    `RibbonThetaVaultMATICCall 10% strikeSelection @ ${strikeSelection.address}`
  );
  ...
  const proxy = await deploy("RibbonThetaVaultMATICCall10", {
    contract: "AdminUpgradeabilityProxy",
    from: deployer,
    args: [logicDeployment.address, admin, initData],
  });

  console.log(`RibbonThetaVaultMATICCall 10 %Proxy @ ${proxy.address}`);
...
main.tags = ["RibbonThetaVaultMATICCall10"];
